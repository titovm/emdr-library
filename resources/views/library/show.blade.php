            ...
            <div class="form-container">
                ...
                <header class="form-header">
                    <div class="flex items-center justify-between mb-6">
                        <a href="{{ route('library.index') }}" class="btn-secondary inline-flex items-center">
                        ...
                        @if(auth()->check() && auth()->user()->is_admin)
                            <div class="flex items-center gap-3">
                                <a href="{{ route('library.edit', $item->id) }}" class="btn-secondary inline-flex items-center">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="space-y-6">
                                    @foreach($videoFiles as $video)
                                        <div class="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                                            <h3 class="font-semibold text-lg mb-4 text-gray-900 dark:text-gray-100">{{ $video->name }}</h3>
                                            <div class="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden">
                                                ...
                                                @elseif(Str::contains($video->external_url, 'vimeo.com'))
                                                    ...
                                                    <iframe src="https://player.vimeo.com/video/{{ $videoId }}" 
                                                        ...
                                                        allowfullscreen class="rounded-lg shadow-lg"></iframe>
                                                @else
                                                    <div class="flex items-center justify-center h-64 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 rounded-lg">
                                        ...
                                        <div class="dynamic-input-group">
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center space-x-4">
                                                    ...
                                                    <div>
                                                        <h3 class="font-semibold text-gray-900 dark:text-gray-100">{{ $file->name }}</h3>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium text-blue-700 dark:text-blue-300">{{ __('app.documents') }}</span>
                                    </div>
                                    <span class="text-blue-600 dark:text-blue-400 font-bold">{{ $documentCount }}</span>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium text-purple-700 dark:text-purple-300">{{ __('app.videos') }}</span>
                                    </div>
                                    <span class="text-purple-600 dark:text-purple-400 font-bold">{{ $videoCount }}</span>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ __('app.created_at') }}</span>
                                    </div>
                                    <span class="text-gray-600 dark:text-gray-400 font-medium">{{ $item->created_at->format('M d, Y') }}</span>
                                </div>
                                ...
                                @if($item->updated_at->ne($item->created_at))
                                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                                        <div class="flex items-center">
                                            ...
                                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ __('app.updated_at') }}</span>
                                        </div>
                                        <span class="text-gray-600 dark:text-gray-400 font-medium">{{ $item->updated_at->format('M d, Y') }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="space-y-2">
                                    @foreach($item->categories as $category)
                                        <a href="{{ route('library.category', $category) }}" 
                                           class="flex items-center p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-colors duration-200 group">
                                            <svg class="w-4 h-4 text-primary-600 dark:text-primary-400 mr-3 group-hover:translate-x-1 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                            </svg>
                                            <span class="font-medium text-primary-700 dark:text-primary-300">{{ $category }}</span>
                                        </a>
                                    @endforeach
                                </div>
                            </div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($item->tags as $tag)
                                        <a href="{{ route('library.tag', $tag) }}" 
                                           class="px-3 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors duration-200">
                                            #{{ $tag }}
                                        </a>
                                    @endforeach
                                </div>
